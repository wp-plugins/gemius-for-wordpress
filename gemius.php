...
define( 'GEMIUSWP_VERSION', '0.1' );

if ( is_admin() && ! class_exists( 'TLA_GemiusWP_Admin' ) ) {

	load_plugin_textdomain( 'gemiuswp', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );

	class TLA_GemiusWP_Admin {

		public function gemiuswp_options_page() {
			?>
			<div class="wrap">
		...
		public function gemiuswp_section_text() {
			echo '<p>Log in to Gemius and find the Gemius Identifier and enter it below.</p>';
		}

		public function gemiuswp_setting_string() {
		...
		public function gemiuswp_admin_init() {
			register_setting( 'gemiuswp_options', 'gemiuswp_options' );
			add_settings_section('gemiuswp_main', 'Gemius Settings', array($this,'gemiuswp_section_text'), 'gemiuswp' );
			add_settings_field('gemiuswp_text_string', 'Gemius Identifier', array($this,'gemiuswp_setting_string'), 'gemiuswp', 'gemiuswp_main');
		}

		public function gemiuswp_admin_add_page() {
		...
		public function __construct() {
			...
			$options = tla_gemiuswp_get_options();
...
function tla_gemiuswp_get_options() {
	$options = get_option('gemiuswp_options');
	return $options;
}
